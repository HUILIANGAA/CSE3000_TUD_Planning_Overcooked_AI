...
class CoupledPlanningExperimentalAgent(Agent):
    """
    ...
    def action(self, state):
        try:
            ...
            stateInfo = err.args[1]
            kabouter = str(stateInfo).partition("facing")
            direction = kabouter[2][1:8]
            print("direction I got ", direction)
            # opposite
            if "(0, -1)" in direction: return Direction.SOUTH
            if "(0, 1)" in direction: return Direction.NORTH
            if "(1, 0)" in direction: return Direction.WEST
            if "(-1, 0)" in direction: return Direction.EAST



            print("failures: ", self.mlp.failures)
...
class CoupledPlanningAgent(Agent):
    ...
    def action(self, state):
        try:
            ...
            print("COUPLED PLANNING FAILURE")
            self.mlp.failures += 1
            print("failures: ", self.mlp.failures) #added
            return Direction.ALL_DIRECTIONS[np.random.randint(4)]
